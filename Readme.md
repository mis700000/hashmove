...
Explore
 ...
 Sign out
 Watch 9
  Star 21  Fork 14 agilecrm/rest-api
 Code  Issues 5  Pull requests 1  Projects 0  Wiki  Insights
Branch: master Find file Copy pathrest-api/README.md
2c33b1d  on Oct 16
@ranjanagilecrm ranjanagilecrm Update README.md
3 contributors @saikiran-k @graut @ranjanagilecrm
RawBlameHistory     
4770 lines (4357 sloc)  155 KB
Agile CRM REST API
...
9 Delete single contact
10 Search contact by email
10.A Search contact by email
10.B Search contact by email
11 Search contacts/companies
12 Adding tags to a contact based on email
13 Delete tags to a contact based on email
14 Add score to a contact using email ID
15 Get tasks related to contact
16 Updating contact properties by Email
17 Change contact owner
18 Add contact to a campaign
19 Remove contact from a campaign
20 Get contact by phone number
21 Get contacts by dynamic filter
21.A Get contacts by tag filter
21.B Get contacts by custom field
Company APIs
1 Creating a company
2 Updating a company
3 Get list of companies
4 Get company by id
5 Delete single company
Deals
...
Deal APIs
1 Listing deals
2 Get deal by its ID
3 Create deal
4 Partial update
5 Create deal to a contact using email ID
6 Delete deal
7 Bulk delete
8 Get deals from default track grouped by milestones
9 Get deals for a particular track (grouped by milestone)
10 Get deals from particular track
11 Get deals related to specific contact
12 Get deals of current user (my deals)
13 Remove contacts of a deal
Notes
...
8 Delete notes from specific deal
Tasks

Task fields
Task APIs
1 Get the list of pending tasks depending on the number of pending days
2 Get the list of tasks based on given filters
3 Get the task based on ID
4 Create a task
5 Create a task based on Contact email
6 Update a task (Partial update)
7 Delete a task based on ID
Events

Event fields
Event APIs
1 Get list of events
2 Get events related to contact
3 Create event
4 Update event
5 Delete an event
Track / Milestones
...
1 Get documents related to specific contact
2 Create a document to a contact
3 Update a document to a contact
Help Desks

1 Get all tickets
2 Create a ticket
3 Delete a ticket
4 Get all filter IDs
5 Get all messages within a ticket
Video references

Create contact test
Update contact test
Update tag by ID test
Create deal test
Get list of contacts example 1
Get list of contacts example 2
Things to know:
...
Finding API Key, domain and email
...
Using this API you can not delete properties.If subtype is same for phone,website or email then value can be overridden. Lead score, star value and tags can not be updated using this API. follow the below API for editing lead score,star value and tags.
...
###dev/api/contacts/edit/add-star Method: PUT
...
Method: GET - Gets the deal with the given ID.
...
Method: DELETE - Deletes the note of the specific contact. (It will remove the relationship between the note and the contact.)
...
Method: POST - Deletes notes of the specific deal.
...
```sh` curl https://{domain}.agilecrm.com/dev/api/milestone/pipelines -H "Content-Type : application/json" -H "Accept : application/json" -d ‘{"name" : "Test" , "milestones" : "Open,New,Prospect,Proposal,Won,Lost" }’ -v -u {email} : {API Key} -X POST
